---
name: Getting started
---

import { FiAirplay, FiAlertOctagon, FiAnchor } from 'react-icons/fi'
import {
    Input,
    CheckBoxGroup,
    TextArea,
    Labelled,
    Select,
    Slider,
    Switch,
    CheckBox,
    CardOptions,
    Form,
} from 'form-blocks/src'
import { useFormState, useField } from 'react-final-form'
import {
    ThemeProvider,
    CSSReset,
    Box,
    Stack,
    useColorMode,
    Button,
    ColorModeProvider,
} from '@chakra-ui/core'

import { Playground } from 'dokz'

# Form Blocks

Form Blocks is a set of components built to create beautiful forms

<Playground>
    <ThemeProvider>
        <Stack align='center'>
            <CSSReset />
            <Form>
                <Stack p='40px' maxWidth='700px' spacing='40px'>
                    <Labelled
                        name='input'
                        sublabel='description of input'
                        field={<Input />}
                        label='Label'
                    />
                    <Labelled
                        name='checkbox'
                        field={<CheckBox children='single one' />}
                    />
                    <Labelled
                        name='checkboxgroup'
                        label='textArea'
                        field={
                            <CheckBoxGroup
                                items={[{ value: 'cosa' }, { value: 'altro' }]}
                            />
                        }
                    />
                    <Labelled name='area' field={<TextArea />} label='labell' />
                    <Labelled
                        name='select'
                        label='select an option'
                        field={
                            <Select
                                items={[{ value: '1' }, { value: '2' }]}
                                isRequired
                            />
                        }
                    />
                    <Labelled
                        name='slider'
                        label='select an option'
                        field={<Slider max={10} name='slide' />}
                    />
                    <Labelled
                        name='switch'
                        label='switch this'
                        field={<Switch />}
                    />
                    <Labelled
                        name='cards'
                        label='Select your card'
                        sublabel='with high attention'
                        field={
                            <CardOptions
                                options={[
                                    {
                                        icon: <FiAirplay />,
                                        value: 'x',
                                        label: 'The x option is the first',
                                        sublabel:
                                            'Sublabels add text no one reads',
                                    },
                                    {
                                        icon: <FiAlertOctagon />,
                                        value: 'y',
                                        label: 'The y option',
                                        sublabel: 'maybe true',
                                    },
                                    {
                                        icon: <FiAnchor />,
                                        value: 'z',
                                        label: 'The Z makes sleep',
                                        sublabel:
                                            'Sublabels add text no one reads',
                                    },
                                ]}
                                name='sdf'
                            />
                        }
                    />
                </Stack>
            </Form>
        </Stack>
    </ThemeProvider>
</Playground>
